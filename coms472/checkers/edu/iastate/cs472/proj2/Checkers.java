...
public class Checkers extends JPanel {
    ...
    public static class PreBoard extends JPanel{
    	...
    	public void drawBoard(CheckersData currentBoard)
    	{
    		premessage = new JLabel("",JLabel.LEFT);
    		premessage.setFont(new  Font("Serif", Font.BOLD, 14));
    		premessage.setForeground(Color.green);
    		premessage.setText("Black (agent) to play");
    ...
    private class Board extends JPanel implements ActionListener, MouseListener {
        ...
        //   current player.
        AdversarialSearch player; // AI player
        /**
        ...
        public void decideAIplayer()
        {
            ...
            while (!done) {
                try {
                    ...
                    if (aikey==1) {player = new AlphaBetaSearch(); done = true; aiKey = 1;}
                    else if(aikey==2) 
                    {player = new MonteCarloTreeSearch(); done =true; aiKey= 2;}
                    else if(aikey==3) 
                    {
                    	player = new MonteCarloTreeSearch(); done =true; aiKey= 3;}
                    else {
        ...
        void doMakeMove(CheckersMove move) {	
            board.makeMove(move);
            CheckersData preBoard= copyBoard(board);
        	previous.drawBoard(preBoard);
            repaint();
            ...
            if (currentPlayer == CheckersData.RED) {
                ...
                if(aiKey==3)
                {
                	...
                	moveAI = player.legalMoves()[rand.nextInt(2)];
                }
